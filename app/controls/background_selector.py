...
BACKGROUND_LOOKUP     = {bg.code        : bg.description for bg in BACKGROUND_TOTAL}
BACKGROUND_LOOKUP_REV = {bg.description : bg.code        for bg in BACKGROUND_TOTAL}
...
class background_selector(section):
    def __init__(self,parentFrame:tk.Frame=None,r:int=0,c:int=0):
        ...
        self.selection = tk.StringVar(value=BACKGROUND_LOOKUP[DEFAULT_BACKGROUND])
        self.value     = tk.StringVar(value=DEFAULT_BACKGROUND)
    ...
    def on_changed(self,bgIdx:str=None):
        selected_code = bgIdx if bgIdx != None else BACKGROUND_LOOKUP_REV[self.selection.get()]
        self.value.set(selected_code)
        selected_description = BACKGROUND_LOOKUP[self.value.get()]
